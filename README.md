...
右手先到 10 cm 的小坐标轴，再到 18 cm 的大坐标轴并闭合手指。若 HUG 腕部
距烧杯超过 25 cm，该轮在运动前即标记为规划无效：机器人安全保持，周期结束后
写入失败并自动 reset，不会再因异常退出整个采集进程。6D IK 的单步关节目标变化
限制为 0.02 rad，避免异常旋转把手臂瞬间推到关节限位。
...
调试抓取几何时可用 `--object-shape box|cylinder|sphere` 替换烧杯，并可用
`--hand-closure-scale` 调节闭合幅度。`--debug-ik` 会打印关键阶段的末端目标、
位置误差和雅可比预测位移。
...
下一轮 HUG 规划。IK 使用持久关节目标，避免执行器跟踪滞后导致未完成动作被下一步覆盖。
...
`tools/retarget_calibrator` 会将当前 `1.45` 掌长比例对应的相对位姿作为初始值，叠加
显示半透明 canonical MANO 开手网格和 Unitree 官方 Inspire URDF 零关节网格。可用
