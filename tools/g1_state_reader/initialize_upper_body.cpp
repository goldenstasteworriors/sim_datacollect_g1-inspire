...
int main(int argc, char** argv) {
  if (argc != 2 && argc != 3) {
    std::cerr << "Usage: g1_initialize_upper_body NETWORK_INTERFACE\n";
    return 2;
  }
  if (argc == 3) {
    std::cerr << "Robot-side execution is disabled. Run lab-g1-arm-player on the PC.\n";
    return 8;
  }
  ...
  std::cout << "]\n";
  std::cout << "No command publisher exists in this robot-side tool.\n";
  return 0;
}
